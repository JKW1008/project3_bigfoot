...
export const ContainCourse = async (user_id, table_name, course_id, course_name, lat, lon, qr) => {
    const isCourseExist = await IntroduceRepository.Existcourse(user_id, course_id, course_name);
  
    if (!isCourseExist) {
      // 데이터가 존재하지 않으면 저장
      await IntroduceRepository.containCourse(user_id, table_name, course_id, course_name, lat, lon, qr);
      return true;
    } else {
